...
function render(manifest, assets) {
  const visualGroups = similarGroups(assets);
  summary.hidden = false;
  groups.hidden = false;
  grid.hidden = false;
  summary.innerHTML = `<div><strong>${assets.length}</strong><span>retained assets</span></div><div><strong>${manifest.assets.length}</strong><span>discovered in Manifest</span></div><div><strong>${visualGroups.length}</strong><span>possible visual groups</span></div>`;
